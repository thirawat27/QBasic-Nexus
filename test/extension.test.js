...
suite("QBasic Nexus Extension Test Suite", () => {
  ...
  test("Language support should be registered", async () => {
    ...
    assert.ok(
      languages.includes("qbasic"),
      "QBasic language should be registered",
    )
  })

  test("Commands should be registered", async () => {
    const commands = await vscode.commands.getCommands(true)
    const qbasicCommands = commands.filter((cmd) =>
      cmd.startsWith("qbasic-nexus."),
    )

    assert.ok(
      qbasicCommands.length > 0,
      "QBasic Nexus commands should be registered",
    )
    assert.ok(
      qbasicCommands.includes("qbasic-nexus.compile"),
      "Compile command should exist",
    )
    assert.ok(
      qbasicCommands.includes("qbasic-nexus.compileAndRun"),
      "Compile & Run command should exist",
    )
  })
})
...
suite("Compiler Module Tests", () => {
  ...
  test("Transpiler should transpile simple PRINT statement", () => {
    ...
    assert.ok(
      ...
      "Transpiled code should contain the string",
    )
    assert.ok(
      result.includes("_print"),
      "Transpiled code should use _print function",
    )
  })
  ...
  test("Transpiler should handle SUB definitions", () => {
    ...
    assert.ok(
      result.includes("async function MySub"),
  ...
  test("Transpiler should handle FUNCTION definitions", () => {
    ...
    assert.ok(
      result.includes("async function Add"),
      "Should generate async function",
    )
