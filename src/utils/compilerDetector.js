...
const { IS_WINDOWS, IS_MACOS, IS_LINUX } = require('./platform');
...
async function getCompilerVersion(compilerPath) {
    try {
        ...
        // Check for Phoenix
        const isPhoenix = output.toLowerCase().includes('phoenix') || 
...
async function detectCompiler(options = {}) {
    ...
    for (const method of searchMethods) {
        try {
            ...
            console.error(`[CompilerDetector] ${method.name} search failed:`, err.message);
        }
    }
