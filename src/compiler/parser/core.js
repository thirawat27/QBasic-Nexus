...
module.exports = {
  ...
  _emitVar(name, init, comment = '') {
    const suffix = comment ? ` ${comment}` : '';
    this._emit(`var ${name} = ${init};${suffix}`);
  },
...
const _MOUSEINPUT = _mouseinput;
const _MOUSEX = _mousex;
const _MOUSEY = _mousey;
const _MOUSEBUTTON = _mousebutton;
...
const _resetFiles = _runtime.resetFiles || _closeAll;
const _memcopy = _runtime.memcopy || (() => {});
const _memfill = _runtime.memfill || (() => {});
...
const _filesFunc = _runtime.files || (() => {});
...
async function _open(filename, mode, filenum) {
  ...
  let data = _virtualFiles[key] || '';
  if (mode === 'OUTPUT') data = '';
  _openFiles[filenum] = { filename: key, mode, data, pos: 0, eof: data.length === 0 };
}
...
async function _close(filenum) {
  ...
  const fh = _openFiles[filenum];
  if (fh && (fh.mode === 'OUTPUT' || fh.mode === 'APPEND')) {
    _virtualFiles[fh.filename] = fh.data;
  }
  delete _openFiles[filenum];
}

async function _closeAll() {
  // If runtime supports closeAll
  for (const filenum in _openFiles) {
    await _close(filenum);
  }
}

async function _listFiles(spec) {
  if (_runtime.files) {
    await _filesFunc(spec);
    return;
  }

  const names = Object.keys(_virtualFiles);
  _print('Files in VFS:', true);
  if (names.length === 0) {
...
async function _printFile(filenum, text) {
  if (_runtime.printFile) {
    await _printFileFunc(filenum, text);
    return;
  }
  const fh = _openFiles[filenum];
  if (!fh) throw new Error("Bad file number");
  fh.data += String(text);
  fh.eof = fh.pos >= fh.data.length;
}

async function _inputFile(filenum) {
  if (_runtime.inputFile) {
    return _inputFileFunc(filenum);
  }
  const fh = _openFiles[filenum];
  if (!fh) throw new Error("Bad file number");
  if (fh.pos >= fh.data.length) {
    fh.eof = true;
    return '';
  }
  let end = fh.data.indexOf('\\n', fh.pos);
  if (end === -1) end = fh.data.length;
  let line = fh.data.slice(fh.pos, end);
  if (line.endsWith('\\r')) line = line.slice(0, -1);
  fh.pos = end < fh.data.length ? end + 1 : end;
  fh.eof = fh.pos >= fh.data.length;
  return line;
}

function _lset(target, value) {
  const width = String(target ?? '').length;
  if (width <= 0) return '';
  return String(value ?? '').padEnd(width).slice(0, width);
}

function _rset(target, value) {
  const width = String(target ?? '').length;
  if (width <= 0) return '';
