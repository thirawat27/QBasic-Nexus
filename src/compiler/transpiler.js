...
class Parser {
  ...
  _parseStatement() {
    // Handle Native Declarations and forward declarations
    if (this._matchKw("DECLARE")) {
      if (this._matchKw("DYNAMIC") && this._matchKw("LIBRARY")) {
        return this._parseDeclareLibrary(true)
      } else if (this._matchKw("LIBRARY")) {
        return this._parseDeclareLibrary(false)
      } else if (this._matchKw("JAVASCRIPT")) {
        return this._parseDeclareJavascript()
      } else {
  ...
  _parseDeclareLibrary(isDynamic) {
    ...
    if (libName) {
      this._emit(`// Load external Native Library: ${libName}`)
      this._emit(
        `const _lib_${libName.replace(/[^a-zA-Z0-9_]/g, "")} = typeof require !== 'undefined' ? require("${libName}") : (typeof window !== 'undefined' ? window["${libName}"] : {});`,
      )
    }

    while (!this._isEnd() && !this._checkKw("END")) {
      this._skipNewlines()
      if (this._checkKw("END")) break

      if (this._matchKw("SUB") || this._matchKw("FUNCTION")) {
        const id = this._consume(TokenType.IDENTIFIER)
        if (!id) break
        const name = id.value
        ...
        if (this._matchKw("ALIAS")) {
          if (this._check(TokenType.STRING)) {
            aliasName = this._advance().value
          }
        }

        if (libName) {
          const safeLib = "_lib_" + libName.replace(/[^a-zA-Z0-9_]/g, "")
          this._emit(
            `const ${name} = (...args) => ${safeLib} ? ${safeLib}.${aliasName}(...args) : null;`,
          )
        } else {
          this._emit(
            `const ${name} = (...args) => typeof ${aliasName} !== 'undefined' ? ${aliasName}(...args) : null;`,
          )
        }
        // Register function in globals so parser can call it safely
        BUILTIN_FUNCS[name.toUpperCase()] = name
      } else {
    ...
    if (this._matchKw("END")) {
      this._matchKw("DECLARE")
    }
  }

  _parseDeclareJavascript() {
    this._emit("// --- NATIVE JAVASCRIPT BLOCK ---")
    let jsCode = ""
    ...
    while (!this._isEnd()) {
      ...
      if (t.type === TokenType.NEWLINE) {
        ...
        // Because token boundaries might strip spaces, we add a space logic
        const s = ["(", ")", ",", ".", "[", "]", "="].includes(t.value)
          ? ""
          : " "
        jsCode += s + t.value
      }
    ...
    for (const line of jsCode.split("\\n")) {
      ...
      if (trimmedLine) {
        // Basic hack to restore Javascript formatting roughly
        this._emit(
          trimmedLine
            .replace(/ \(/g, "(")
            .replace(/\( /g, "(")
  ...
  _parseMul() {
    ...
    while (true) {
      ...
      else if (this._matchOp("\\"))
        left = `Math.floor(${left} / ${this._parseUnary()})`
      else if (this._matchOp("^"))
        left = `Math.pow(${left}, ${this._parseUnary()})`
