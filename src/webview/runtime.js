...
;(function () {
  ...
  async function vfsInput(filenum, numItems = 1) {
    // Advanced token-based input supporting CSV formatting
    const fh = fileHandles[filenum]
    if (!fh) return Array(numItems).fill("")
    ...
    while (results.length < numItems) {
      if (fh.inputBuffer.length > 0) {
        results.push(fh.inputBuffer.shift())
      } else {
        // Read next line
        ...
        const line = lines[fh.position++]
        // Simple CSV parser for QBasic (ignores quoted commas)
        const tokens = []
        let inQuotes = false
        let currentToken = ""
        ...
        tokens.push(currentToken.trim()) // Add last token
        fh.inputBuffer.push(...tokens)
      }
    }
